...
function wave2()
{
	...
	camSetVtolData(CAM_THE_COLLECTIVE, "vtolAppearPos", "vtolRemoveZone", list, camChangeOnDiff(camMinutesToMilliseconds(3)), (difficulty >= INSANE) ? CAM_REINFORCE_CONDITION_BASES : "COCommandCenter", ext);
}
...
function vtolAttack()
{
	if (camClassicMode())
	{
		...
		camSetVtolData(CAM_THE_COLLECTIVE, "vtolAppearPos", "vtolRemoveZone", list, camChangeOnDiff(camMinutesToMilliseconds(2)), (difficulty >= INSANE) ? CAM_REINFORCE_CONDITION_BASES : "COCommandCenter", ext);
	}
	else
	{
