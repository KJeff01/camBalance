...
function wave2()
{
	...
	camSetVtolData(CAM_THE_COLLECTIVE, (difficulty >= INSANE) ? undefined : "vtolAppearPos", "vtolRemoveZone", list, camChangeOnDiff(camMinutesToMilliseconds(4)), (difficulty >= INSANE) ? CAM_REINFORCE_CONDITION_ARTIFACTS : "COCommandCenter", ext);
}
...
function vtolAttack()
{
	...
	{
		const list = [cTempl.colcbv, cTempl.colcbv];
		camSetVtolData(CAM_THE_COLLECTIVE, (difficulty >= INSANE) ? undefined : "vtolAppearPos", "vtolRemoveZone", list, camChangeOnDiff(camMinutesToMilliseconds(3)), (difficulty >= INSANE) ? CAM_REINFORCE_CONDITION_ARTIFACTS : "COCommandCenter");
	}
	else
	{
